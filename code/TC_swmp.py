...
if __name__ == "__main__":
    ...
    while(1):
        ...
        s1.cell(cnt, 17).value = (str(datetime.now())[:16])  # 儲存格 B1 內容 ( row=1, column=2 ) 為 100
        s1.cell(cnt, 18).value = str(gym_peop[0])  # 儲存格 B2 內容 ( row=2, column=2 ) 為 200
        s1.cell(cnt, 19).value = str(swim_peop[0])  # 儲存格 B3 內容 ( row=3, column=2 ) 為 300
        cnt += 1
